...
void* load_train_thread(void *args)
{
	...
	station_load_train(ltargs->station, ltargs->fseats);
...
int main()
{
	...
	int i;
	const int Total_pssgs = 100;
	int pssgs_left = Total_pssgs;
	for (i = 0; i < Total_pssgs; i++) {
		pthread_t tid;
		int ret = pthread_create(&tid, NULL, pssg_thread, &station);
		if (ret != 0) {
			// If this fails, perhaps we exceeded some system limit.
			// Try reducing 'Total_pssgs'.
	...
	while (pssgs_left > 0) {
		...
		int threads_to_reap = MIN(pssgs_left, fseats);
		...
		pssgs_left -= threads_reaped;
		Total_pssgs_boarded += threads_reaped;
